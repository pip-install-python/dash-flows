...
Keyword arguments:
- `data` (required): Node data configuration object containing display and behavior settings. data has the following type: lists containing elements 'label', 'title', 'sublabel', 'body', 'icon', 'iconColor', 'showIcon', 'layout', 'multiline', 'style', 'className', 'handleStyle', 'sourcePosition', 'status', 'loadingVariant', 'smartHandles'.
Those elements have the following types:
  ...
  - `smartHandles` (Bool; optional): Enable smart handles mode - renders source handles on all 4 sides for optimal edge routing
